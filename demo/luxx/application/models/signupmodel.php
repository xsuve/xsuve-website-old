...
class SignUpModel {
  ...
  // Create Account
  public function signupAccount($name, $email, $phone_number, $password) {
    if(!empty($name) && !empty($email) && !empty($password)) {
      ...
      if($accounts_with_email == 0) {
        $sql = "INSERT INTO luxx_accounts (name, email, phone_number, password, register_date, activated, activate_token) VALUES (:name, :email, :phone_number, :password, :register_date, :activated, :activate_token)";
        $query = $this->db->prepare($sql);
        $query->execute(array(':name' => $name, ':email' => $email, ':phone_number' => $phone_number, ':password' => $password, ':register_date' => $register_date, ':activated' => 0, ':activate_token' => $activate_token));
